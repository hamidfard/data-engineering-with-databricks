...
products_df = spark.read.option("header", True).option("inferSchema", True).csv(products_csv_path)
...
products_df2 = spark.read.option("header", True).schema(user_defined_schema).csv(products_csv_path)
